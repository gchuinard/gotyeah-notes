...
- [x] ~~**MCP — sprints (côté Sonar)**~~ : fait (2026-06-30). 4 outils `notes_*_sprint` (list/create/update/delete, `update` gère démarrer/terminer + renvoi des incomplètes au backlog si `database_id`) + param `sprint` (par NOM, ou `"backlog"`) sur create/update_record, dans `gotyeah_sonar`. Logique pure testée (16/16). ⚠️ **À déployer** : push `gotyeah_sonar` sur `main` + rafraîchir le connecteur Sonar dans claude.ai.
