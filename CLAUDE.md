...
| `KEYCLOAK_ADMIN_CLIENT_ID` / `_SECRET` | `lib/keycloak.ts` | vides | Provisioning des invités dans l'IdP **désactivé** — sans effet aujourd'hui, aucune route n'appelle `ensureInvitedUser` (les invités entrent par le lien email). ⚠️ Client dédié au realm applicatif, `manage-users` + `view-users` uniquement |
