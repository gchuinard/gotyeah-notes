...
- **L'invitation s'ACCEPTE (08/08/2026)** — ajouter quelqu'un ne l'ajoute plus, ça le lui **propose**.
  ...
  - ⚠️ **CINQ points d'appel, pas trois.** Login, les DEUX branches du callback OIDC, et les DEUX branches de `consumeMagicLink`. La doctrine d'origine en annonçait trois — elle avait été écrite avant le lien de connexion par email. Le chemin qui fait RÉELLEMENT entrer un invité externe est le lien magique, pas le login : un changement qui l'oublie referme la seule porte d'entrée, et la CI ne le dit pas.
...
⚠️ **« Mes espaces » n'est PAS « les espaces que le MCP rend ».** Le pont incarne `ia@gotyeah.local`, et depuis le rattachement d'office ce compte est membre d'espaces où Gautier ne l'est pas (l'espace personnel d'un autre compte, par exemple). `notes_list_workspaces` porte donc un champ **`yours`** — vrai quand le porteur du jeton IdP est lui-même membre —, calculé sur l'**email vérifié**, jamais sur le displayName. **Quand l'utilisateur dit « mes espaces », ne retenir que `yours: true`, et n'écrire dans un `yours: false` que s'il l'a nommé explicitement.** `yours: null` = on n'a pas pu savoir : demander, jamais supposer.
