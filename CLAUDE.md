...
RecordComment   → COMMENTAIRE d'une carte. APPEND-ONLY : pas d'updatedAt, pas de
                  deletedAt, AUCUNE route de modification ni de suppression (décision du
                  09/08). recordId → Cascade, authorId → SetNull (l'écrit survit au compte,
                  l'affichage dégrade en « Quelqu'un »). Corps en TEXTE BRUT, 4000 car.
                  ⚠️ Sa raison d'être : Record.content s'écrit en REMPLACEMENT TOTAL, donc
                  une remarque posée dans le corps risque d'être effacée — c'est le piège
                  des « 🗣️ Notes de Gautier ». Ici chaque message a sa ligne.
                  ⚠️ AUCUNE notification : le message porterait un extrait de texte, et le
                  prévenir à qui ne peut pas ouvrir la carte divulguerait ce contenu.
...
- **Commentaires (`RecordComment`)** — un fil par carte, `GET`/`POST /api/records/[id]/comments`, onglet « Commentaires » du `RecordPanel` (monté à la demande, comme l'Historique).
  ...
  - **MCP** : `notes_list_record_comments` et `notes_add_record_comment` (dépôt `gotyeah-mcp`). Sans eux, un commentaire serait écrit pour un lecteur — la session IA suivante — qui ne le verrait jamais. `update`/`delete` sont des `Gap` motivés dans `notes_entities.py`.
...
- ⚠️ **UN SEUL fetcher, `lib/client/fetcher.ts`. N'en écris JAMAIS un autre** (unifié le 09/08/2026). Le projet en définissait **19**, dont **11 sans tester `res.ok`** — et ce n'était pas cosmétique : sur un 401/404 à corps JSON (session expirée, carte en corbeille), `data` recevait `{ error: … }`, que SWR traite comme une donnée valide. Le défaut `= []` ne s'applique PAS (un objet n'est pas `undefined`), donc `data.map(…)` levait **en rendu**. Sans `error.tsx` ni ErrorBoundary dans `src/app`, le repli est le GlobalError de Next, qui **remplace le root layout** : barre latérale et en-tête disparaissent. `WorkspaceContext` tuait ainsi l'application entière à l'expiration de session.
  - **`HttpError` porte `.status`**, sinon distinguer « session expirée » de « disparu » obligerait à re-parser un message. Le **corps de l'erreur n'est jamais lu** : le récupérer inviterait à l'afficher, c'est-à-dire à retomber dans le défaut. `loadErrorMessage(err)` rend la phrase — jamais un code nu.
  - ⚠️ **`noRetryOn4xx` sur toute clé qui peut légitimement répondre 4xx.** Le défaut de SWR relance en boucle avec backoff : on réessaie une panne, pas un refus. C'est ce qui rend sûre la bascule de `blocknoteSchema`, dont la dégradation en libellé « page » sur un lien mort est **voulue**.
  - ⚠️ **L'ORDRE DE RENDU EST IMPOSÉ : erreur → chargement → vide → liste.** Deux raisons. (a) Un « Aucun élément » affiché alors que le chargement a ÉCHOUÉ est un second mensonge : sur les pièces jointes, le composant vient d'apprendre à l'utilisateur qu'un retrait est **définitif**, donc « Aucun document » se lit comme une suppression. (b) **SWR efface l'erreur du cache dès qu'un `mutate()` réussit** — si la liste passe avant, publier un commentaire fait disparaître la bannière et laisse un fil d'un seul message, complet en apparence.
  - ⚠️ Dans un contexte monté en permanence (`WorkspaceContext`), l'écran d'échec se déclenche sur `error && data === undefined`, **jamais sur `error` seul** : un échec de REVALIDATION laisse la liste précédente et l'app utilisable, alors qu'un `error` nu ferait clignoter tout l'écran au moindre hoquet réseau.
...
- **Renommage inline du titre d'une carte kanban : DOUBLE-clic** (09/08/2026). Le clic simple ouvre la carte, comme partout ailleurs sur elle. ⚠️ **Le report de l'ouverture (`DOUBLE_CLICK_DELAY_MS`, 300 ms) est load-bearing, ne le « simplifie » pas** : un double-clic émet **deux `click` AVANT le `dblclick`**, donc sans lui le premier ouvrirait déjà le panneau et le renommage démarrerait derrière. L'arbitrage tient dans un seul handler grâce à `e.detail` (2 = second clic du geste), qui arrive avant `dblclick`. Le délai ne porte **que sur le titre** — partout ailleurs sur la carte, l'ouverture reste immédiate — et **pas en lecture seule**, où il n'y a rien à départager. Deux branches, deux tests (`e2e/kanban-title-dblclick.spec.ts`) : sans celui du clic simple, on « corrigerait » en le supprimant et le titre deviendrait une **zone morte**. L'input porte `data-card-title-input` : ⚠️ ne le cible jamais par `input[value='…']`, ce sélecteur cesse de correspondre dès la première frappe.
...
- ⚠️ **TOUTE écriture doit RÉCONCILIER LE CACHE SWR, le corps compris** (corrigé le 09/08/2026, défaut observé en production). `RecordPanel` resème ses éditeurs depuis le cache à **chaque montage** — `useCreateBlockNote({ initialContent })` ne lit sa graine qu'une fois, `sections` s'initialise dans un `useState`, et les **cinq** vues montent le panneau avec `key={record.id}` : rouvrir une carte est un montage neuf. `saveTitle` et `saveProperty` réconciliaient déjà ; les deux savers du corps, non. Fermer puis rouvrir **sans recharger** réaffichait donc le corps d'AVANT la saisie.
  - ⚠️ **Le symptôme trompe** : l'utilisateur rapporte « ça ne s'enregistre pas », alors que le texte est bien en base — un Ctrl+R le ramène. C'est le test qui tranche : si le rechargement rend le texte, le fautif est le cache, pas la sauvegarde. Un correctif visant à « fiabiliser l'autosave » passerait à côté.
  - ⚠️ **Et le second temps DÉTRUIT** : taper dans l'éditeur périmé renvoie le document périmé en **remplacement TOTAL**. Sur une carte sectionnée, toutes les sections d'un coup, « 🗣️ Notes de Gautier » comprise — le piège que la doctrine décrit pour l'API, réalisé par l'UI. La coalescence 2 min des révisions fusionne l'aller et le retour dans la même ligne : le texte perdu n'existe alors **nulle part**.
  - **Réconciliation LOCALE** (`globalMutate(key, updater, { revalidate: false })`), jamais un refetch : la clé records rend TOUS les records corps compris (plusieurs centaines de Ko sur un board réel), la relire à chaque demi-seconde de frappe échangerait un bug contre une lenteur sur le Pi. ⚠️ Un échec (`!res.ok`) ne touche PAS le cache : l'y écrire ferait servir un mensonge à la réouverture.
  - **`pagehide` flushe les saisies en attente** : fermer l'onglet ou recharger ne démonte pas React, donc le nettoyage de `useEffect` ne s'exécute pas. C'est le `keepalive: true` des savers qui rend cet envoi de dernière seconde délivrable. `pagehide` et non `beforeunload` — ce dernier ne se déclenche pas sur mobile.
  - **Le panneau AFFICHE son état** (`data-save-state` : Enregistrement… / Enregistré ✓ / Non enregistré), comme `Editor.tsx` le fait pour les pages. Son absence est ce qui a laissé le défaut vivre : rien ne contredisait l'utilisateur. L'échec est **rouge et persistant**, il ne retombe pas sur « idle ».
  - ⚠️ **Piège de test, la vraie leçon du lot** : le spec de non-régression existant rouvrait la carte après un `page.goto()` — un rechargement dur qui **vide le cache SWR**. Il ne pouvait donc structurellement pas voir le défaut, tout en ayant l'air de le garder. Un test d'autosave doit rouvrir **sans recharger** (`e2e/autosave.spec.ts`, « SANS recharger »), et se vérifier ROUGE sur le code d'avant.
...
- **CD** : `.github/workflows/deploy.yml` — déclenché par `workflow_run` quand la CI d'un push sur `main` réussit (ou par `workflow_dispatch`, qui ne part que de `main` et exige un run CI vert sur le commit), SSH sur le Pi (secrets repo `SSH_HOST`/`SSH_USER`/`SSH_KEY`), `git reset --hard` sur le commit testé + `docker compose up -d --build`, puis attend que le conteneur `gotyeah_notes` soit `healthy` (healthcheck node défini dans `docker-compose.yml`). ⚠️ **Un push sur `main` à la CI verte déclenche un déploiement réel.** Jusqu'au 25/09/2026, le déploiement partait au push, en parallèle de la CI, sur `origin/main` : un commit aux tests rouges partait donc en production. Deux gardes dans le script : si `main` a avancé depuis le commit testé, rien n'est fait (le suivant sera déployé après sa CI, la prod ne recule jamais) ; si seuls des `.md` ont changé depuis la version en ligne, pas de reconstruction (rôle tenu auparavant par `paths-ignore: ["**.md"]` sur le push). Un commit mêlant doc et code déploie quand même.
...
### Les 52 outils `notes_*` (état du code, `remote.py`)
...
- **Déploiement du MCP** : après tout ajout d'outil, l'ordre est **push `main` → redéploiement du conteneur → PUIS rafraîchissement du connecteur claude.ai**. Sauter le dernier temps produit un outil bien présent en production que le client ne propose pas, et qu'on relit à tort comme « pas déployé ».
  ...
  - ⚠️ Corollaire pour `gotyeah-notes`, qui se déploie autrement (`git reset --hard` sur le commit testé + rebuild) : **là**, le HEAD du Pi est légitime. La règle n'est donc pas « le HEAD ment », c'est « vérifier au bon endroit selon le mode de déploiement du dépôt ». Les deux dépôts n'ont pas le même.
...
**La règle qui s'applique aujourd'hui, sans exception : jamais de push sur `main` sans go explicite de Gautier**, parce qu'un push sur `main` déclenche un déploiement réel en production (dès que sa CI est verte ; seuls les changements ne touchant **que** des `.md` ne reconstruisent rien, par une garde du script de `deploy.yml` qui remplace l'ancien `paths-ignore`). Unique dérogation accordée à ce jour : l'**auto-merge des petits correctifs low-risk** (retours de recette, tweaks UI sans surface données ni sécurité), une fois la **CI verte**. Le go explicite reste requis pour l'auth, les données, les migrations, le schéma Prisma, la sécurité et tout changement de taille.
