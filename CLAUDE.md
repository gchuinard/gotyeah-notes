...
- **50 outils depuis le 05/08** : `notes_list_members` (userId/displayName/role, **sans email**) + résolution des assignés PAR NOM dans `notes_create_record`/`notes_update_record` (propriété `user` : displayName, email ou userId acceptés ; liste des membres chargée seulement si un assigné est visé).
