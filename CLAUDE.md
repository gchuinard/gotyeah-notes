...
- **Comptes SSO depuis Réglages → Membres (07/08/2026)** — voir *qui peut se connecter par l'IdP*, créer le compte manquant, suspendre ou rétablir un accès. `GET /api/workspaces/[id]/idp` (état) + `POST /api/workspaces/[id]/members/[userId]/idp` (`create` | `suspend` | `resume`), **admin de l'espace** les deux.
  ...
  - Non configuré (`KEYCLOAK_ADMIN_*` vides) ⇒ `configured: false` et **aucun bouton affiché** : pas de bouton fantôme qui « réussit » sans rien faire. La route répond 409 de son côté.
