...
- **Invitation par EMAIL + inscription sur invitation (05/08/2026)** — l'invitation cesse d'être muette, et devient le **laissez-passer** du provisioning.
  ...
  - **Provisioning de l'invité DANS l'IdP (`lib/keycloak.ts`, 06/08)** — sans lui le parcours était **cassé pour un externe** : le realm a l'auto-inscription fermée (et doit le rester), donc l'email disait « connecte-toi » à quelqu'un qui ne *pouvait* pas. `POST /members` crée désormais le compte Keycloak via l'API admin, et Keycloak envoie son propre lien d'activation. ⚠️ **Surface volontairement étroite** : client dédié `notes-provisioning` **créé DANS le realm `gotyeah`** (un client de `master` n'obtiendrait aucun jeton sur l'issuer du realm, et voisinerait l'administration de tous les realms), portant `manage-users` + `view-users` et **rien d'autre** (jamais `realm-admin`), aucun flux de connexion, un unique point d'entrée `ensureInvitedUser` appelé depuis la seule route d'invitation (déjà gatée admin + `INVITE_BUDGET`). ⚠️ **`emailVerified: false` et aucun mot de passe** — load-bearing : la preuve de possession vient du clic sur le lien envoyé *à cette adresse*. Le poser à `true` ferait qu'un admin se trompant d'adresse fabriquerait un compte « vérifié » pour une boîte qu'il ne contrôle pas, alors que le claim d'invitation repose justement sur `email_verified`. ⚠️ `existing` **n'envoie aucun email d'activation** : proposer un nouveau mot de passe à qui en a déjà un ressemble à une prise de contrôle. **Retour dans l'app (06/08)** : l'email d'activation porte `client_id` + `redirect_uri` = `<APP_BASE_URL>/api/auth/oidc/login` — pas `/`, car la session Keycloak vient d'être ouverte, donc le flux OIDC est silencieux et dépose l'invité **connecté** (viser la racine imposerait un aller-retour par l'écran de connexion). ⚠️ **Prérequis côté IdP** : cette URL doit figurer dans les *Valid redirect URIs* du client `notes` — vérifié le 06/08, elle n'y était PAS (seul le callback l'est, sans joker). D'où le **repli obligatoire** : un 400 sur l'envoi avec redirection le rejoue **sans**, car un réglage absent de ce dépôt ne doit pas priver l'invité de tout email — on aurait troqué un parcours perfectible contre un parcours impossible. ⚠️ **`/admin/*` est fermé côté public** (Cloudflare Access → **302**, pas 401 : un échec au diagnostic trompeur), d'où `KEYCLOAK_INTERNAL_URL` = `http://login-keycloak:8080`, les deux conteneurs partageant le réseau `nginx-proxy-manager_default`. Seule la **base** est surchargeable : le **realm reste dérivé d'`OIDC_ISSUER`** — provisionner dans un autre realm que celui qui authentifie ne se verrait qu'au moment où l'invité, compte créé, ne pourrait pas se connecter. Vides = désactivé, on revient au geste manuel, et l'écran Membres le dit.
...
| `KEYCLOAK_ADMIN_CLIENT_ID` / `_SECRET` | `lib/keycloak.ts` | vides | Provisioning des invités dans l'IdP **désactivé** — le compte Keycloak se crée à la main. ⚠️ Client dédié au realm applicatif, `manage-users` + `view-users` uniquement |
| `KEYCLOAK_INTERNAL_URL` | `lib/keycloak.ts` | vide → base d'`OIDC_ISSUER` | Base interne pour l'API admin (`http://login-keycloak:8080`). ⚠️ Requise ici : `/admin/*` public est derrière Cloudflare Access (302). Le realm n'est **jamais** surchargé |
